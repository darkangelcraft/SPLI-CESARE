...
def dictionary_exist
	exist_f=File.exist?("frequency_dictionary.txt")
	exist_w=File.exist?("words_dictionary.txt")
	if(exist_f)
...
def create_fdictionary
	f=File.new("frequency_dictionary.txt","w")
	tmp=nil
...
def create_wdictionary
	...
	f=File.new("words_dictionary.txt","w")
	str.scan(/\w+/) {|word|
...
def decrypt(offset)
	char=''
	result=""
	str=File.read("STORY_CIPHER.txt")
...
def frequency_attack(flag)
	if(flag==false)
		...
		h1=calculate_frequencies("STORY_CIPHER.txt")
		h2=calculate_frequencies("frequency_dictionary.txt")
		...
		f=File.new("frequency_decrypted_text.txt","w")
